...
function mergeArrays(arr1: number[], arr2: number[]): number[] | number {
    // Function to find the middle value of a combined array
    function findMiddleValue(combineArr: number[]): number {
        let middleIndex = Math.floor(combineArr.length / 2);
        return combineArr[middleIndex];
    }

    let sameLength = arr1.length === arr2.length;
    let allElementsSame: boolean = true;
...
let names = ["John", "Alice", "Bob", "David", "Eve"];
